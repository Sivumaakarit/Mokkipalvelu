...
export function DemoBadge() {
    ...
    return (
        <>
            ...
            `}</style>
            <div
                className={`fixed top-[74px] md:top-[92px] left-3 md:left-6 z-50 bg-white/95 backdrop-blur-sm border border-dashed border-[#0A1128]/35 shadow-md rounded-xl p-2 md:p-3 flex flex-col gap-1.5 items-center transition-all duration-500 cursor-default select-none pointer-events-auto origin-top-left hover:scale-105 animate-slow-blink ${isHidden ? 'opacity-0 -translate-x-full pointer-events-none' : ''}`}
            >
                ...
                <span className="text-[8px] md:text-[10px] uppercase tracking-[0.2em] font-black text-[#0A1128] leading-none px-0.5">DEMOSIVU</span>
            </div>
        </>
    );
}
